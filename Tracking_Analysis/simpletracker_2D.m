function [ tracks, tracks_no_t_c, tracks_time_coord ] = simpletracker_2D(points,time_per_frame, varargin)
...
%     time_per_frame = 25;
    ...
    for i_trajectory = 1:trajectory_num
        tracks_time_coord_nan{i_trajectory,1} = zeros(ideal_length_traj,3);
        for j_trajectory_step = 1:ideal_length_traj
            if ~isnan(tracks_no_t_c{i_trajectory,1}(j_trajectory_step,1))
                tracks_time_coord_nan{i_trajectory,1}(j_trajectory_step,1) = time(j_trajectory_step);
                 tracks_time_coord_nan{i_trajectory,1}(j_trajectory_step,2:3) = points{j_trajectory_step,1}(tracks_no_t_c{i_trajectory,1}(j_trajectory_step,1),:);
            else
